...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MASTERat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMASTERsend
enum ZerocoinSpendStatus {
    ZMASTER_SPEND_OKAY = 0,                            // No error
    ZMASTER_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMASTER_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMASTER_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMASTER_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMASTER_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMASTER_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMASTER_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMASTER_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMASTER_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMASTER_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMASTER_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMASTER_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMASTER_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMASTER_SPENT_USED_ZMASTER= 14,                      // Coin has already been spend
    ZMASTER_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMASTER_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZMasterBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMASTEROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmasterAuto) const;
    ...
    */
    ...
    CzMASTERWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMASTERTracker> zmasterTracker;
    ...
    void setZWallet(CzMASTERWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmasterTracker = std::unique_ptr<CzMASTERTracker>(new CzMASTERTracker(strWalletFile));
    }

    CzMASTERWallet* getZWallet() { return zwalletMain; }
    ...
    void setZMasterAutoBackups(bool fEnabled)
    {
    ...
    /** zMASTERreset */
    boost::signals2::signal<void()> NotifyzMASTERReset;
