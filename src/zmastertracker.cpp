...
CzMASTERTracker::CzMASTERTracker(std::string strWalletFile)
{
...
CzMASTERTracker::~CzMASTERTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMASTERTracker::Init()
{
...
bool CzMASTERTracker::Archive(CMintMeta& meta)
{
...
bool CzMASTERTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMASTERTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMASTERTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMASTERTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMASTERTracker::GetSerialHashes()
{
...
CAmount CzMASTERTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMASTERTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMASTERTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMASTERTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMASTERTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMASTERTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMASTERTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMASTERTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMASTERTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMASTERTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMASTERTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMASTERTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMASTERTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMASTERTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMASTERTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMASTERTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMASTERTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmaster from DB\n", __func__, listDeterministicDB.size());
    }
