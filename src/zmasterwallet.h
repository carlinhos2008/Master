...
#ifndef MasterStake_ZMASTERWALLET_H
#define MasterStake_ZMASTERWALLET_H
...
public:
    CzMASTERWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMASTER(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMASTER(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MasterStake_ZMASTERWALLET_H
