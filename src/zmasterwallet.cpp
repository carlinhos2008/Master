...
CzMASTERWallet::CzMASTERWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmaster seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMASTERSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMASTERSeed_deprecated()) {
                    LogPrintf("%s: Updated zMASTERseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmaster seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmaster wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMASTERWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMASTERCount(nCountLastUsed);
    else if (!walletdb.ReadZMASTERCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMASTERWallet::Lock()
{
    seedMaster = 0;
}

void CzMASTERWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMASTERWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMASTER(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMASTERWallet::LoadMintPoolFromDB()
{
...
void CzMASTERWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMASTERWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMASTERWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmasterTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMASTERWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMASTER(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmasterTracker which also adds to database
    pwalletMain->zmasterTracker->Add(dMint, true);
...
void CzMASTERWallet::SeedToZMASTER(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMASTERWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMASTERWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZMASTERCount(nCountLastUsed);
...
void CzMASTERWallet::GenerateDeterministicZMASTER(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMASTERWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMASTERWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
