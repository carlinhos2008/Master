...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MasterStake address (e.g. %1)").arg("Bdec03SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no MasterStake: URI
...
{
    // Convert masterstake:// to masterstake:
    //
    //    Cannot handle this later, because masterstake:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open masterstake.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MasterStake.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MasterStake.lnk
...
{
    return GetAutostartDir() / "masterstake.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a masterstake.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MasterStake\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the masterstake app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add masterstake app to startup item list
