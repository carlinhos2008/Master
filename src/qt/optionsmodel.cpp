...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMasterStakeAmount"))
        settings.setValue("nAnonymizeMasterStakeAmount", 1000);

    nAnonymizeMasterStakeAmount = settings.value("nAnonymizeMasterStakeAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMasterStakeAmount"))
        SoftSetArg("-anonymizemasterstakeamount", settings.value("nAnonymizeMasterStakeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in masterstake.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterStakeAmount:
            return QVariant(nAnonymizeMasterStakeAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterStakeAmount:
            nAnonymizeMasterStakeAmount = value.toInt();
            settings.setValue("nAnonymizeMasterStakeAmount", nAnonymizeMasterStakeAmount);
            emit anonymizeMasterStakeAmountChanged(nAnonymizeMasterStakeAmount);
            break;
        case CoinControlFeatures:
