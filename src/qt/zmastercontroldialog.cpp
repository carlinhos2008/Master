...
// Copyright (c) 2017-2018 The MasterStake Developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zmastercontroldialog.h"
#include "ui_zmastercontroldialog.h"
...
std::set<std::string> ZMasterControlDialog::setSelectedMints;
std::set<CMintMeta> ZMasterControlDialog::setMints;

ZMasterControlDialog::ZMasterControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZMasterControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMasterControlDialog::~ZMasterControlDialog()
{
    delete ui;
}

void ZMasterControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMasterControlDialog::updateList()
{
...
void ZMasterControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMasterControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMaster_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMasterControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMasterControlDialog::GetSelectedMints()
{
...
void ZMasterControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
