...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered masterstake address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 MASTER.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal masterstake:URI (masterstake:XyZ...?message=example)
            {
...
static void NotifyzMASTERReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMASTERReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMASTERReset.connect(boost::bind(NotifyzMASTERReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMASTERReset.disconnect(boost::bind(NotifyzMASTERReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmasterTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
