...
#include "config/masterstake-config.h"
#endif
...
CzMASTERWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("masterstake-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "masterstake.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "masterstaked.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MASTERand zMASTERmoney supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 49266, 27293));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MASTER/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MASTER/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, masterstake, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MASTER/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MasterStake specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-masterstake=<n>", strprintf(_("Enable or disable staking functionality for MASTERinputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmasterstake=<n>", strprintf(_("Enable or disable staking functionality for zMASTERinputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:49266"));
    ...
    strUsage += HelpMessageOpt("-backupzmaster=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMASTERminting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmasterbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMASTERbackup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizemasterstakeamount=<n>     " + strprintf(_("Keep N MASTERanonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 49265, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The MasterStake Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("masterstake-loadblk");
...
/** Sanity checks
 *  Ensure that MasterStake is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MasterStake is shutting down."));
...
#endif
    // Make sure only a single MasterStake process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MasterStake is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MasterStake version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //MasterStake: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MasterStake specific: zerocoin and spork DB's
                ...
                // MasterStake: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMASTERMinted();
                        RecalculateZMASTERSpent();
                    }
                    RecalculateMASTERSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // MasterStake: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MasterStake") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MasterStake to complete") << "\n";
        ...
        zwalletMain = new CzMASTERWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zMASTERWallet
        uiInterface.InitMessage(_("Syncing zMASTERwallet..."));

        bool fEnableZMasterBackups = GetBoolArg("-backupzmaster", true);
        pwalletMain->setZMasterAutoBackups(fEnableZMasterBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zmasterTracker->Init();
...
//    }
//
//    nAnonymizeMasterStakeAmount = GetArg("-anonymizemasterstakeamount", 0);
//    if (nAnonymizeMasterStakeAmount > 999999) nAnonymizeMasterStakeAmount = 999999;
//    if (nAnonymizeMasterStakeAmount < 2) nAnonymizeMasterStakeAmount = 2;
    ...
    LogPrintf("Anonymize MasterStake Amount %d\n", nAnonymizeMasterStakeAmount);
    ...
    /* Denominations
       ...
       For example:
       1MASTER+1000 == (.1MASTER+100)*10
       10MASTER+10000 == (1MASTER+1000)*10
    */
