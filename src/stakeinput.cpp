...
CZMasterStake::CZMasterStake(const libzerocoin::CoinSpend& spend)
{
...
int CZMasterStake::GetChecksumHeightFromMint()
{
...
int CZMasterStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZMasterStake::GetChecksum()
{
    return nChecksum;
}

// The zMASTERblock index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZMasterStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CZMasterStake::GetValue()
{
    return denom * COIN;
}

//Use the first accumulator checkpoint that occurs 60 minutes after the block being staked from
bool CZMasterStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CZMasterStake::GetUniqueness()
{
    //The unique identifier for a zMASTERis a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZMasterStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZMasterStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zMASTERthat was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMASTEROutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMASTERoutput", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMASTER", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMASTEROutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMASTERoutput", __func__);
...
bool CZMasterStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZMasterStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzMASTERTracker* zmasterTracker = pwallet->zmasterTracker.get();
    CMintMeta meta;
    if (!zmasterTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmasterTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
